...
console.log('Disabling conditional user configured requirements...');
...
// Find and disable all "Condition - user configured" executions
const conditions = executions.filter(e => e.providerId === 'conditional-user-configured' && e.requirement === 'REQUIRED');

console.log(`Found ${conditions.length} conditional user configured executions to disable`);

for (const condition of conditions) {
  console.log(`Disabling condition: ${condition.id}`);
  ...
  if (!updateResp.ok) {
    console.error(`Failed to disable condition ${condition.id}:`, await updateResp.text());
  } else {
    console.log(`Successfully disabled condition ${condition.id}`);
  }
}

console.log('All conditional requirements disabled');