...
console.log('Disabling cookie execution to ensure clean redirect...');
...
// Find the cookie execution
const cookieExecution = executions.find(e => e.providerId === 'auth-cookie');
if (!cookieExecution) {
  console.error('Cookie execution not found');
  process.exit(1);
}

console.log(`Found cookie execution: ${cookieExecution.id}, current requirement: ${cookieExecution.requirement}`);
...
console.log('Cookie update response status:', updateResp.status);
if (!updateResp.ok) {
  console.log('Cookie update error:', await updateResp.text());
} else {
  console.log('Successfully disabled cookie execution');
}