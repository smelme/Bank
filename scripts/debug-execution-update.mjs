...
console.log('Username password execution details:');
console.log(JSON.stringify(usernamePasswordExecution, null, 2));

// Try to update using the correct endpoint
console.log('\nTrying to update execution...');
const updateResp = await fetch(base + `/authentication/executions/${usernamePasswordExecution.id}/config`, {
  method: 'PUT',
  headers: {
...
if (!updateResp.ok) {
  console.log('Update response body:', await updateResp.text());
} else {
