import dotenv from 'dotenv';
dotenv.config();

const KEYCLOAK_URL = process.env.KEYCLOAK_URL;
const REALM = process.env.KEYCLOAK_REALM;
const ADMIN_CLIENT_ID = process.env.KEYCLOAK_ADMIN_CLIENT_ID;
const ADMIN_CLIENT_SECRET = process.env.KEYCLOAK_ADMIN_CLIENT_SECRET;

if (!KEYCLOAK_URL || !REALM || !ADMIN_CLIENT_ID || !ADMIN_CLIENT_SECRET) {
  console.error('Missing Keycloak admin env vars');
  process.exit(2);
}

async function getAdminToken() {
  const tokenUrl = `${KEYCLOAK_URL}/realms/${encodeURIComponent(REALM)}/protocol/openid-connect/token`;
  const params = new URLSearchParams();
  params.append('grant_type', 'client_credentials');
  params.append('client_id', ADMIN_CLIENT_ID);
  params.append('client_secret', ADMIN_CLIENT_SECRET);

  const r = await fetch(tokenUrl, { method: 'POST', body: params });
  if (!r.ok) {
    const t = await r.text();
    throw new Error('Token error: ' + r.status + ' ' + t);
  }
  const j = await r.json();
  return j.access_token;
}

async function createTokenExchangeComponent() {
  const token = await getAdminToken();
  console.log('Obtained admin token (len):', token.length);

  const componentsUrl = `${KEYCLOAK_URL}/admin/realms/${encodeURIComponent(REALM)}/components`;

  const jwksUrl = process.env.ORCHESTRATOR_JWKS_URL || `https://bank-production-37ea.up.railway.app/.well-known/jwks.json`;

  // Create a token exchange component
  const componentPayload = {
    name: 'orchestrator-token-exchange',
    providerId: 'token-exchange',
    providerType: 'org.keycloak.protocol.oidc.TokenExchangeProvider',
    config: {
      'jwks-url': [jwksUrl],
      'issuer': ['https://bank-production-37ea.up.railway.app']
    }
  };

  console.log('Creating token exchange component with payload:', JSON.stringify(componentPayload, null, 2));

  const res = await fetch(componentsUrl, {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer ' + token,
      'Content-Type': 'application/json'
    },
    body: JSON.stringify(componentPayload)
  });

  const text = await res.text();
  console.log('Status:', res.status);
  console.log('Response body:', text.slice(0, 2000));

  if (!res.ok) {
    throw new Error('Failed to create component: ' + res.status + ' ' + text);
  }

  console.log('Token exchange component created successfully');
}

(async () => {
  try {
    await createTokenExchangeComponent();
    console.log('Component creation finished');
  } catch (err) {
    console.error('Error creating component:', err.message);
    process.exit(1);
  }
})();